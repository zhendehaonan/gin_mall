...
func (dao *ProductImgDao) ListProductImg(productId uint) (imgs []*model.ProductImg, err error) {
	err = dao.DB.Model(&model.ProductImg{}).Where("product_id = ?", productId).Find(&imgs).Error
	return
}
